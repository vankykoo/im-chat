    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out){
        ...
        if (isProtobuf(in)) {
            ...
            if (!map.containsKey("protobufDecoder")){
                //添加protobuf解码器
                ctx.pipeline().addAfter("serverMultiProtocolDecoder", "protobufDecoder",
                        new ProtobufDecoder(BaseMsgProto.BaseMsg.getDefaultInstance()));
                ctx.pipeline().addAfter("serverMultiProtocolDecoder", "protobufVarint32FrameDecoder",
            ...
            if (!map.containsKey("stringDecoder")){
                //添加 string 解码器
                ctx.pipeline().addAfter("serverMultiProtocolDecoder", "stringDecoder",
                        new StringDecoder());
            }
